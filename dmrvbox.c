/*
    DMRVBox - DMR Personal Voice Box with listening via email attachment and/or radio listening
    Copyright (C) 2024 IU4QTF

    Based on code from:
    DMRVMsg https://github.com/narspt/DMRVMsg
...
int		host1_tg;
const char *	host1_pw;
const char *	host1_url;
const char *	host2_url;
int 	host1_port;
int 	host2_port;
...
int process_connect(int connect_status, char *buf)
{
	...
	case DMR_AUTH:
		...
		sprintf(&out[8], "%-8.8s%09u%09u%02u%02u%8.8s%9.9s%03d%-20.20s%-19.19s%c%-124.124s%-40.40s%-40.40s", callsign,
				435000000, 435000000, 1, 1, latitude, longitude, 0, "Nowhere","Italy", '4', "www.google.com", "20210101", "MMDVM"); // 302 bytes
		len = 302;
...
ini_t *config = ini_load(cnf);


const char *b;
const char *bul = "true";

u_radio = ini_get(config, "opts", "use_radio");

b = u_radio;

if (strcmp(b, bul) == 0)
{
	printf("use_radio: %s", u_radio);
	use_radio = true;
	

}

u_email = ini_get(config, "opts", "use_email");

b = u_email;

if (strcmp(b, bul) == 0)
{
	printf("use_email: %s", u_email);
	use_email = true;

}

u_sql = ini_get(config, "opts", "use_sql");
    ...
    if (use_sql == true)
      {
        ...
        sql_server = ini_get(config, "mysql", "sql_server");

        sql_usr = ini_get(config, "mysql", "sql_usr");

        sql_pw = ini_get(config, "mysql", "sql_pw");

        sql_db = ini_get(config, "mysql", "sql_db");
		

      }

 
callsgn = ini_get(config, "dmr", "callsign");



 memset(callsign, ' ', strlen(callsgn));
 memcpy(callsign, callsgn, strlen(callsgn));

id = ini_get(config, "dmr", "dmrid");
dmrid = atoi(id);
host1_url = ini_get(config, "dmr", "dmrhost");
dmrport = ini_get(config, "dmr", "dmrport");
host1_port = atoi(dmrport);
dmrtg = ini_get(config, "dmr", "dmrtg");
host1_tg = atoi(dmrtg);
host1_pw = ini_get(config, "dmr", "dmrpw");
host2_url = ini_get(config, "dmr", "ambeip");

ambeport = ini_get(config, "dmr", "ambeport");
host2_port = atoi(ambeport);

save_path = ini_get(config, "dmr", "save_path");
reply_path = ini_get(config, "dmr", "reply_path");
...
void load_email_ini(const char* cnf)
{

ini_t *config = ini_load(cnf);
        
		const char * smtp_port;
        email_from = ini_get(config, "email", "from");
        email_to = ini_get(config, "email", "to");
        smtp_server = ini_get(config, "email", "smtp_server");
        smtp_port = ini_get(config, "email", "smtp_port");
        smtp_pt = atoi(smtp_port);
        smtp_usr = ini_get(config, "email", "smtp_usr");
        smtp_pw = ini_get(config, "email", "smtp_pw");
        

ini_free(config);
}

void load_sql_ini(const char* cnf)
{
...
void sqlinsert(const char * cnf, char* rx_srcid, char* rx_callsign, char* daterec, char* timerec, char* filename)
{
  
     
   char *dbinsert = "INSERT INTO messages (fromcall,fromdmrid,date,time,pathfile,listened,advsent) VALUES (";
   char *dbcommit = ",'0','0');";


char sqlcmd[4000];

sprintf(sqlcmd, "%s \"%s\",\"%s\",%s,%s,\"%s\"%s", dbinsert, rx_callsign, rx_srcid, daterec, timerec, filename, dbcommit);  
...
load_sql_ini(cnf);
 ...
 /* SQL messages table
`  `fromcall` varchar(45) DEFAULT NULL,
  `fromdmrid` varchar(45) NOT NULL,
  `date` date NOT NULL,
  `time` time NOT NULL,
  `pathfile` varchar(80) NOT NULL,
  `listened` tinyint(4) DEFAULT NULL,
  `advsent` tinyint(4) DEFAULT NULL, <-not used at this moment
*/


     if (!mysql_real_connect(conn, sql_server,
         sql_usr, sql_pw, sql_db, 0, NULL, 0)) {
      fprintf(stderr, "%s\n", mysql_error(conn));
      exit(1);
   }
...
int sql_getlist(const char* cnf)
{

//load the struct and return the number of messages not yet listened

char query[256]; 

sprintf(query, "SELECT idmessages, pathfile FROM messages where listened = 0;");
...
if (!mysql_real_connect(conn, sql_server, sql_usr, sql_pw, sql_db, 0, NULL, 0)) {
      fprintf(stderr, "%s\n", mysql_error(conn));
      return -1;
   }
...
if (mysql_query(conn, query) != 0) {
  fprintf(stderr, "MySQL query exec error: %s\n", mysql_error(conn));
...
sprintf(rd[idx].id, "%s", row[0]);
sprintf(rd[idx].fname, "%s", row[1]);
...
void sql_update(char *index, const char* cnf)
{

char query[256]; 

sprintf(query, "UPDATE messages SET listened = 1 WHERE idmessages = %s;", index);
...
void send_notification(char* filename)
{

  quickmail_initialize();
  quickmail mailobj = quickmail_create(email_from, "DMRVbox Notification");
  quickmail_add_to(mailobj, email_to);

  quickmail_add_header(mailobj, "Importance: Low");
  quickmail_add_header(mailobj, "X-Priority: 5");
  quickmail_add_header(mailobj, "X-MSMail-Priority: Low");
  quickmail_set_body(mailobj, "Hi,\n you have a new message.\n");
 
  quickmail_add_body_memory(mailobj, "text/html", "Hi,<br> you have a new message.<br>\n", 40, 0);

  quickmail_add_attachment_file(mailobj, filename, NULL);
...
const char* errmsg;
  ...
  if ((errmsg = quickmail_send(mailobj, smtp_server, smtp_pt, smtp_usr, smtp_pw)) != NULL)
...
int main(int argc, char **argv)
{
	...
	char wavelan[20];            //it = italiano, en = inglese
	int waveinfo = 0;           //Returned message: 0 (rt_diff <=3) = Welcome, 1 = Thanks
	char srcid[20];
    char pfx_i1[] = "222"; //I
	char pfx_i2[] = "223"; //I
	char pfx_i3[] = "224"; //I
	char pfx_t77[] = "292"; //RSM
	...
	*(exelastslash+1) = '\0';
	...
	if( (argc <= 1)  ){
		...
		fprintf(stderr, "Usage: dmrvbox -i <config.ini>     load configuration from file\n               -v                print release\n");
	...
	if (strcmp(argv[1], "-v") == 0){
      printf("DMRVbox Personal voice box. (c) 2024 IU4QTF Ver: %s\n", VERSION);
	  return 0;
...
loadini(in);
	...
	sql_t rd[100];
        ...
        printf("CALL: %s\nTG: %d\nDMRID: %d\n", callsign, host1_tg, dmrid);
	}    

	char recpath[4096] = {0};
	char voicepath[4096] = {0};


	if (strlen(save_path) < sizeof(recpath)-1) 	strcpy(recpath, save_path);
	if (strlen(recpath) == 0) strcpy(recpath, ".");
	if (recpath[strlen(recpath)-1] != '/') 	recpath[strlen(recpath)] = '/';

    if (strlen(reply_path) < sizeof(voicepath)-1) 	strcpy(voicepath, reply_path);
	if (strlen(voicepath) == 0) strcpy(voicepath, ".");
	if (voicepath[strlen(voicepath)-1] != '/') 	voicepath[strlen(voicepath)] = '/';
    



	printf("Save recordings to: %s\n", recpath);
	printf("Reply messages are under: %s\n", voicepath);
        ...
        if ( (FrameType == DMRMMDVM_FRAMETYPE_DATASYNC) /*&& (CallType == 1)*/ ) {
          if ((buf[15] & 0x0F) == MMDVM_SLOTTYPE_HEADER) {
           ...
           rt_start = time(NULL); 
            ...
            sprintf(daterec, "%d%02d%02d", ptm->tm_year+1900, ptm->tm_mon+1, ptm->tm_mday);
			sprintf(timerec, "%02d%02d%02d", ptm->tm_hour, ptm->tm_min, ptm->tm_sec);


			sprintf(filename, "%s%0s-%s.%03d_%d_%s.wav", recpath, daterec, timerec, tv.tv_usec / 1000,  rx_srcid, rx_callsign);
...
#endif		
        rt_stop = rx_endt;
        rt_diff = (rt_stop - rt_start);
		
        printf ("EPOCHDIFF: %d seconds\n", (rt_stop - rt_start));

		
          }
        }

        else if ( ((FrameType == DMRMMDVM_FRAMETYPE_VOICE) || (FrameType == DMRMMDVM_FRAMETYPE_VOICESYNC)) && (rt_diff >= 3)/*&& (CallType == 1)*/ ) {
         /*Send to ambeserver for conversion only if the rx from dmr is equal or more than 3 seconds*/
    ...
    if (rx_endt && (time(NULL) > rx_endt)) { //rx end
        ...
        if (rx_wavefile != NULL) {
          ...
          rx_wavefile = NULL;
		  /*remove file less than 3 seconds long*/
		  if(rt_diff <= 3){
		  remove(filename);
	  } 
...
//self tx: check via radio if there are new messages to me and let the app transmit it if true.

		  if ((use_radio) && ((tx_tgid == rx_srcid) || (host1_tg == rx_srcid))){

			
            newmsg = false;
			selftx = true;
            int ret = sql_getlist(in);
            
              msgnr = ret;            
			 if ((ret == -1) || (ret == 0))
             {
             printf("No rows on sql select\n");
			 msgnr = 0;
			 //return message nn_en/it.wav
        ...
        char wave_tx[100];
		sprintf(srcid, "%d", rx_srcid);
		char *p1 = strstr(srcid, pfx_i1);
        char *p2 = strstr(srcid, pfx_i2);
        char *p3 = strstr(srcid, pfx_i3);
		char *p4 = strstr(srcid, pfx_t77);

         if ((p1 != NULL) || (p2 != NULL) || (p3 != NULL) || (p4 != NULL)) {
			
           /*italian*/
		   sprintf(wavelan, "%s", "_it.wav");
          } else 
		  {
          /*english*/
		  sprintf(wavelan, "%s", "_en.wav");
          }
         ...
         if (rt_diff <= 3){
         /*Instruction (0) or Thankyou (1) wave file*/
		 waveinfo = 0;
          } else 
		  {
          waveinfo = 1;
          }
		  sprintf(wave_tx, "%s%s%d%s", voicepath, "tx", waveinfo, wavelan);
		  } 
		  
		  if((selftx) && (!newmsg)) //first answerback selftx = true;
		  {
           ...
           sprintf(wave_tx, "%s%s%s", voicepath, px, wavelan);
		  }		 
			     

		
		if ((selftx) && (newmsg)) //send unlistened records
		  {
		    
           sprintf(wave_tx, "%s", rd[msgidx].fname);

         if(msgcnt < msgnr) {
		  sql_update(rd[msgidx].id, in);
		 }
                // test if wav exixts otherways send the error message
                tx_wavefile = fopen(wave_tx , "rb");
                if (tx_wavefile == NULL) {
					
            			sprintf(wave_tx, "%s%s%s", voicepath, "err", wavelan);
		  ...
		  msgcnt++;
		   if(msgcnt > msgnr)
		   {
			...
			// send the closing message
              sprintf(wave_tx, "%s%s%s", voicepath, "nx", wavelan);
		   }
	  ...
	  printf("*** TX START ***\n");
		...
		if ((rt_diff > 3) && (use_sql == true)) 
                {
		sprintf(rx_src, "%d", rx_srcid);
		
                sqlinsert(in, rx_src, rx_callsign, daterec, timerec, filename);
		}
        ...
        if (tx_wavefile != NULL) {
          ...
          if ( fread(&ambebuf[6], 1, 320, tx_wavefile) == 320 ) {
           ...
           if ((rt_diff > 3) && (use_email == true) && (!selftx))
